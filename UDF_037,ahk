...
global COLOR_NARKO_BUY := "0xFF64b3ff"
...
;SAMP_CHAT_INFO_OFFSET:="samp.dll"+0x215B2C
...
try {
    ...
    {
        ;MsgBox, 48, API.DLL не был найден. Проверьте подключение библиотеки
    }
} catch() 
{
}
...
m := getCoordinates() 
p := [xv,yv,zv] 
;p := getPlayerPos(getIdByPed(getTargetPed())) 
aa := abs(m[1]-p[1]) 
ab := abs(m[2]-p[2]) 
ac := sqrt(aa*aa + ab*ab) 
alpha := asin(aa/ac) 
...
if (m[1] < p[1]) && (m[2] < p[2]) 
beta := -alpha - 1.57 
writeFloat(hGTA, 0xB6F258, beta) 
}


readGameText() {
Str := readString(hGTA, 0x0BAAD40, 100)
StringReplace, Str, Str, ~r~, , All
StringReplace, Str, Str, ~g~, , All
StringReplace, Str, Str, ~y~, , All
StringReplace, Str, Str, ~b~, , All
return Str
}
